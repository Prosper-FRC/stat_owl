...
/* --- Emma Change #1 : Vertical Space size */
...
function applyTheme(theme) {
    if (theme === 'dark') {
        ...
        if (mainLogo) { // Check if logo element exists
             mainLogo.src = '../images/thescoutowlb.png'; // Dark mode logo
        }
        ...
        themeToggle.textContent = '☀️'; // Sun icon for light
         if (mainLogo) { // Check if logo element existsloa
             mainLogo.src = '../images/thescoutowl_new.png'; // Light mode logo
        }
...
const savedTheme = localStorage.getItem('scouterTheme') || 'dark'; // Default to light
...
function buildButtons(list){ /* ... unchanged ... */ 
    ...
    for (let i = 0; i < list.length; i++) { 
         ...
         else if (layout === 'rect-4'') 
        {
            ...
            { 
                wrapper.className = 'spacer'; 
                const spacerEl2 = createButton(b, layout); 
                wrapper.appendChild(spacerEl2); 
                
            } 
            else { 
                wrapper.className = 'threeBox'; 
                const btnEl2 = createButton(b, layout); 
                wrapper.appendChild(btnEl2); 
            ...
            { 
                i++; 
                i++; 
                const btnEl3 = createButton(list[i], layout); 
                wrapper.appendChild(btnEl3); 
